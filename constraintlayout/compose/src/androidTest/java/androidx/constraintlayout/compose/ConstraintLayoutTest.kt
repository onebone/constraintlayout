...
class ConstraintLayoutTest {
    ...
    fun dividerMatchTextHeight_spread() = with(density) {
        ...
        rule.runOnIdle {
            ...
            Assert.assertEquals(1.dp.toIntPx(), dividerSize.value!!.width)
            ...
            Assert.assertEquals(aspectRatioBoxSize.value!!.height, dividerSize.value!!.height)
        }
    }
    ...
    fun dividerMatchTextHeight_inWrapConstraintLayout_shortText() = with(density) {
        ...
        rule.runOnIdle {
            ...
            Assert.assertEquals(size.toIntPx() * 2, constraintLayoutSize.value!!.width)
            // The height of the ConstraintLayout should be the height of the aspect ratio box.
            Assert.assertEquals(size.toIntPx() / 2, constraintLayoutSize.value!!.height)
            // The aspect ratio gets the requested size.
            Assert.assertEquals(size.toIntPx(), aspectRatioBoxSize.value!!.width)
            // Aspect ratio is preserved.
            Assert.assertEquals(size.toIntPx() / 2, aspectRatioBoxSize.value!!.height)
            // Divider has fixed width 1.dp in constraint set.
            Assert.assertEquals(1.dp.toIntPx(), dividerSize.value!!.width)
    ...
    fun testInspectorValue() {
        rule.setContent {
            ConstraintLayout(Modifier.width(10.dp)) {
                ...
                Assert.assertEquals("constrainAs", modifier.nameFallback)
                Assert.assertNull(modifier.valueOverride)
//                Assert.assert(modifier.inspectableElements.asIterable()).containsExactly(
//                        ValueElement("ref", ref),
//                        ValueElement("constrainBlock", block)
//                )
            }
        }
    }
